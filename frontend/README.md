...
│   ├── friends/ # Система друзей (хуки)<br>
│   ├── messages/ # Сообщения real-time (хуки в useMessages + useSocketSubscription, ui)<br>
│   └── profile/ # Профиль свой-чужой (хуки useUser, useUserPosts)<br>
├── entities/      # Бизнес-сущности<br>
│   ├── friend/    # Сущность Дружба/Заявки (api, types)<br>
│   ├── message/   # Сущность Сообщений (api, types)<br>
...
│   ├── lib/ # Утилиты (работа с localStorage, датой и временем + сокеты)<br>
...
- Созданы легкие провайдеры (QueryProvider, ThemeProvider, RouterProvider, AuthProvider, SocketProvider), которые выполняют ровно одну задачу:<br>
...
данных пользователя и состоянием.</p>
  <p style="text-indent: 50px;">SocketProvider	-	сокеты, подключается только у авторизованных пользователей и отключается при logout</p>
...
react-router-dom.
- Сделан простой Layout в (widgets) с шапкой и боковым меню: шапка сверху, боковое меню слева, контент с использованием MUI, а также в роутеры были изменения, теперь всё
что не login/register работает через Layout, а также стилизация включена через отдельные CSS Modules:
- Защита путей (ProtectedRoute), все остальные маршруты ("/", "/profile", "/friends", "/messages") - обёрнуты (ProtectedRoute), при попытки зайти не
авторизированным перекинит на "/login", но "/login" и "/register" — без защиты (публичные страницы).ProtectedRoute учитывает `isLoading`: при проверке токена показывается
спиннер, исчез «флеш-редирект» на /login при обновлении страницы.
...
- Частино завершенна Auth Flow с обработкой ошибок 401. Ошибка проверки токена больше не дублирует очистку — 401 целиком обрабатывает интерсептор `$api` + событие
`auth:logout`, в редьюсере используется `RESET`.
...
- Создан слой entities/post с маппером(Лента новостей) entities/post; features/feed с хуками usePostsQuery, useCreatePostMutation, useDeletePostMutation,
useLikePostMutation — с оптимистичными обновлениями и откатом при ошибке; PostCard, Feed, SkeletonPost;Создание постов с изображениями (FormData
;Удаление постов)
...
- Профиль поддерживает чужие страницы (`/profile/:id`): стена постов пользователя, дата регистрации; кнопки «Добавить в друзья» (отправка заявки) и «Написать» (открывает чат с пользователем).
- Сообщения (real-time): двухпанельный чат (список диалогов + окно переписки), адаптив; URL-параметры `?userId=` (кнопка «Написать» в профиле) и `?conversation=` (переход из тоста); индикатор «печатает…», автоскролл, подгрузка более ранних сообщений, отправка по Enter.
- Сокет-события чата: `message:new`, `typing`, `conversation:updated`; обработка в `useSocketSubscription` с обновлением кэша TanStack Query.
